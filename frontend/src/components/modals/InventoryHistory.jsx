...
const InventoryHistory = ({ isOpen, onClose, product }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Protocol Ledger: ${product.name}`}>
      <div className="space-y-6">
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-6 bg-blue-500/5 rounded-md border border-blue-500/10">
               <div className="flex items-center gap-4">
                  <div className="p-3 bg-card rounded shadow-sm border border-border">
                     <Package className="w-6 h-6 text-blue-500" />
                  </div>
                  <div>
                     <p className="text-[10px] font-black text-blue-500/60 uppercase tracking-widest">Marketable Yield</p>
                     <h4 className="text-xl font-black text-foreground tabular-nums">{displayStock.toLocaleString()} {product.unit?.toUpperCase()}</h4>
                  </div>
               </div>
            </div>
            <div className="flex items-center justify-between p-6 bg-amber-500/5 rounded-md border border-amber-500/10">
               <div className="flex items-center gap-4">
                  <div className="p-3 bg-card rounded shadow-sm border border-border">
                     <History className="w-6 h-6 text-amber-500" />
                  </div>
                  <div>
                     <p className="text-[10px] font-black text-amber-500/60 uppercase tracking-widest">Salvage / Scrap</p>
            ...
            <table className="w-full text-left">
               <thead>
                  <tr className="bg-muted/30 text-[10px] font-black uppercase text-muted-foreground tracking-widest border-b border-border">
                     <th className="px-6 py-4">Protocol Date / Ref</th>
                     <th className="px-6 py-4">Operational Action</th>
                     <th className="px-6 py-4">Matrix Target</th>
                     <th className="px-6 py-4 text-center">Flux</th>
                     <th className="px-6 py-4 text-right pr-8">Final Reserve</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-border/50">
                  {loading ? (
                     <tr><td colSpan="5" className="p-12 text-center italic text-muted-foreground font-bold uppercase tracking-widest text-[10px]">Synchronizing Protocol Ledger...</td></tr>
                  ) : logs.length === 0 ? (
                     <tr><td colSpan="5" className="p-12 text-center text-muted-foreground font-bold text-[10px] uppercase tracking-widest">No protocol movements detected in registry.</td></tr>
                  ) : logs.map((log) => (
                     <tr key={log._id} className="hover:bg-muted/20 transition-colors">
                        <td className="px-6 py-4">
                           <div className="flex flex-col">
                              <span className="text-[11px] font-black text-foreground">{new Date(log.date).toLocaleDateString()}</span>
