...
const Sidebar = ({ open, setOpen }) => {
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 ${open ? "translate-x-0" : "-translate-x-full"
          }`}
      >
                ...
                return (
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        <Icon
                          className={`w-5 h-5 mr-3 transition-colors duration-200 ${isActive ? "text-blue-600" : "text-gray-400 group-hover:text-gray-600"
                            }`}
                        />
                        {item.name}
                      </>
