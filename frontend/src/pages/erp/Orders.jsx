...
const Orders = () => {
  ...
  const handleExportCSV = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `Order_List_${formatDate(new Date())}.csv`);
    document.body.appendChild(link);
    link.click();
  ...
  const getStatusBadgeOptions = (status) => {
      switch (status) {
        ...
        case "in_progress":
          return { color: "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20", label: "Running", icon: <Activity className="w-3 h-3 mr-1" /> };
        case "invoiced":
        ...
        case "completed":
          return { color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20", label: "Completed", icon: <CheckCircle className="w-3 h-3 mr-1" /> };
        case "cancelled":
    ...
    const renderOrderCard = (o, isPendingPane) => {
      ...
      return (
        <motion.div
          ...
          key={o._id}
          className="bg-card border border-border rounded-md p-4 shadow-sm hover:shadow-md transition-all group flex flex-col gap-3 relative"
        >
    ...
    return (
      <AppLayout>
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-black text-foreground tracking-tight uppercase">Sales Orders</h1>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">View all customer orders and their status</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
            ...
            <div className="p-4 border-b border-border flex flex-col md:flex-row justify-between items-center gap-4 bg-muted/5">
              ...
              <div className="flex items-center gap-4">
                <div className="hidden sm:flex flex-col items-end">
                  <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Total Orders</span>
            ...
            {showFilters && (
              <div className="p-4 bg-muted/10 border-b border-border flex flex-col sm:flex-row gap-4 animate-in slide-in-from-top-2 duration-300">
                <div className="space-y-2">
                  <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Job Status</p>
            ...
            <div className="p-4 bg-muted/5 min-h-[500px]">
              {loading ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-full">
                  {/* LEFT PANE: NEW ORDERS */}
                  <div className="flex flex-col h-full bg-card/50 rounded-md border border-border/50 overflow-hidden shadow-inner">
                  ...
                  {/* RIGHT PANE: ONGOING ORDERS */}
                  <div className="flex flex-col h-full bg-indigo-50/20 dark:bg-indigo-900/10 rounded-md border border-indigo-100 dark:border-indigo-900/30 overflow-hidden shadow-inner">
                    <div className="p-3 border-b border-indigo-100 dark:border-indigo-900/30 bg-indigo-50/50 dark:bg-indigo-900/20 flex items-center justify-between sticky top-0 z-10">
                      <h2 className="text-xs font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 flex items-center gap-2">
                        <Activity className="w-4 h-4" /> Running Orders
                      </h2>
                    ...
                    <div className="p-4 flex-1 overflow-y-auto space-y-4">
                      ...
                      {productionOrders.length === 0 && (
                        <div className="h-full flex items-center justify-center text-indigo-400/50 text-[10px] uppercase tracking-widest font-bold">No Running Jobs</div>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <Modal
          ...
          size="4xl"
          title={editingOrder?._id ? "Edit Order Details" : "Add New Order"}
        >
          <div className="p-4">
            <OrderForm
            ...
            {selectedOrder && (
              <>
                <div className="p-3 bg-primary/5 border border-primary/10 rounded flex items-center justify-between">
                  <div>
                    <p className="text-[10px] text-primary font-bold mb-1 uppercase tracking-widest">Payment Remaining</p>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block ml-1">Extra Details</label>
                  <textarea
                ...
                <div className="flex gap-4 pt-4 border-t border-border">
                  ...
                  >
                    {formLoading ? "Saving..." : `Save: ₹${Number(paymentAmount).toLocaleString()}`}
                  </button>
                </div>
              </>
            )}
        ...
        >
          <div className="p-4">
                  ...
                  return (
                    <div key={item._id} className="relative pl-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{formatDate(item.date)}</span>
                        {isMilestone ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="px-1.5 py-0.5 bg-emerald-500/10 text-emerald-600 rounded text-[9px] font-bold uppercase tracking-widest">Paid</span>
                          </div>
                        )}
            ...
            <div className="flex justify-end mt-4 pt-4 border-t border-border">
              <button onClick={() => setIsHistoryModalOpen(false)} className="erp-button-secondary">Close</button>
            </div>
          </div>
        </Modal>
          ...
          <div className="flex flex-col gap-4 p-4">
            <div className="w-full h-[600px] border border-border rounded overflow-hidden bg-muted/20 shadow-inner">
              {previewUrl ? (
                ...
                <div className="flex items-center justify-center h-full text-muted-foreground text-[10px] font-bold uppercase tracking-[0.3em] animate-pulse">Creating PDF...</div>
              )}
            </div>
        ...
        >
          <div className="p-4 space-y-4">
            {actionOrder && (
              <>
                <div className="p-4 bg-muted/20 border border-border rounded-md text-center space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">About this Order</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                    ...
                    className={`flex items-center gap-3 p-4 rounded-md transition-all group ${actionOrder.status === 'completed' ? 'opacity-50 cursor-not-allowed bg-slate-100 border border-slate-200 grayscale' : 'bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20'}`}
                  >
                    <div className={`w-10 h-10 rounded-md flex items-center justify-center transition-transform ${actionOrder.status === 'completed' ? 'bg-slate-200 text-slate-500' : 'bg-emerald-500/20 text-emerald-600 group-hover:scale-110'}`}>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 p-4 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 rounded-md transition-all group"
                  >
                    <div className="w-10 h-10 rounded-md bg-rose-500/20 flex items-center justify-center text-rose-600 group-hover:scale-110 transition-transform">
                      <Trash2 className="w-5 h-5" />
                    </div>
                    <div className="text-left">
                      <p className="text-sm font-black text-rose-700">Delete Order</p>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-rose-600/70">Delete permanently</p>
                    </div>
                  </button>
                </div>
              </>
