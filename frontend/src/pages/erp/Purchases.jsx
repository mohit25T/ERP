...
import {
   ...
   CheckCircle2, Clock, Layers, X, ExternalLink, Filter, Plus,
...
const Purchases = () => {
   ...
   const handleCreatePurchase = async (e) => {
      ...
      try {
         ...
         setFormData({ supplier: "", material: "", category: "Raw Materials", quantity: 1, taxableAmount: 0, unit: "kg", gstRate: 18, materialGrade: "", thickness: 0, width: 0, length: 0 });
      } catch (err) {
         alert("Error: Check your entries");
      } finally {
   ...
   const handleDeletePurchase = async (id) => {
      if (window.confirm("Delete this purchase record? This will update stock levels.")) {
         try {
   ...
   const fetchPaymentHistory = async (purchaseId) => {
      try {
         ...
         const events = [
            { _id: "birth", date: purchase.createdAt, type: 'milestone', amount: purchase.totalAmount, description: "Order Created" },
            ...res.data.map(p => ({ ...p, type: 'payment' }))
         ];
         if (purchase.status === 'received') {
            events.push({ _id: "completion", date: purchase.receivedAt || purchase.updatedAt, type: 'milestone', amount: 0, description: "All Items Received" });
         }
   ...
   return (
      <AppLayout>
         <div className="space-y-3 pb-20">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-3">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h2 className="text-2xl font-bold text-slate-900 tracking-tight mb-1">Purchase Orders</h2>
                     <p className="text-sm font-medium text-slate-500">View all raw material purchases and payments</p>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
               <div className="p-2.5 bg-white rounded-md border border-slate-100 shadow-sm flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                     <p className="text-sm font-semibold text-slate-500">Total Purchase Value</p>
                  ...
                  <div>
                     ...
                     <p className="text-xs font-medium text-emerald-600 mt-1 flex items-center gap-1">
                        Money spent on materials this month
                     </p>
                  </div>
               </div>
               ...
               <div className="p-2.5 bg-white rounded-md border border-slate-100 shadow-sm flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                     <p className="text-sm font-semibold text-slate-500">Waiting for Goods</p>
               ...
               <div className="p-2.5 bg-primary/5 rounded-md border border-primary/10 shadow-sm flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
            ...
            {/* Data Table Section */}
            <div className="bg-white rounded-md border border-slate-200 shadow-sm overflow-hidden mb-10">
               <div className="p-3 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-3">
               ...
               {/* Filters */}
               {showFilters && (
                  <div className="p-4 bg-slate-50/50 border-b border-slate-100 flex flex-wrap gap-4 items-end animate-in fade-in slide-in-from-top-2">
               ...
               <div className="overflow-x-auto">
                  {loading ? (
                     ...
                     <div className="p-20 flex flex-col items-center justify-center text-slate-500">
                        <div className="w-16 h-16 bg-slate-50 rounded-md flex items-center justify-center mb-4 border border-slate-100">
                                 ...
                                 >
                                    <td>
                                       <div className="flex items-center gap-2">
                                          <div className={`w-8 h-8 rounded-md flex items-center justify-center ${p.status === 'received' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'}`}>
                                             <Box className="w-4 h-4" />
                                          </div>
                                          <div className="flex flex-col">
                                             <span className="text-sm font-bold text-slate-900">PO-{p._id ? p._id.slice(-6).toUpperCase() : "LEGACY"}</span>
                                             <span className="text-xs font-semibold text-slate-500 mt-0.5">{formatDate(p.createdAt)}</span>
                                          </div>
                                       </div>
                                    </td>
                                    <td>
                                       <div className="flex flex-col">
                                          <div className="flex items-center gap-2">
                                             ...
                                             {p.materialGrade && <span className="text-[10px] px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded font-bold border border-indigo-100">{p.materialGrade}</span>}
                                          </div>
                                    ...
                                    <td className="text-center">
                                       <div className="flex flex-col items-center gap-1">
                                          ...
                                          <span className={`px-2.5 py-1 rounded-md text-xs font-semibold border ${p.status === 'received' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                    ...
                                    <td className="text-center">
                                       <div className="flex flex-col items-center gap-1">
                                          ...
                                          <div className="flex flex-col items-center gap-0.5 w-full max-w-[100px]">
                                             <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide border w-full text-center ${p.paymentStatus === 'paid' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                    ...
                                    <td className="text-right">
                                       <div className="flex flex-col items-end gap-0.5">
                                          ...
                                          <span className="text-xs font-medium text-slate-500">With ₹{p.gstAmount?.toLocaleString('en-IN', { minimumFractionDigits: 1 })} GST</span>
                                       </div>
                                    </td>
                                    <td className="text-center">
                                       <div className="flex items-center justify-center gap-1.5">
                                          {p.status === 'pending' && (
                                             ...
                                             >
                                                <CheckCircle2 className="w-3 h-3" /> Stock Received
                                             </button>
                                          )}
         ...
         <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Purchase">
            <form onSubmit={handleCreatePurchase} className="p-4 space-y-4">
               <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                     <label className="text-xs font-semibold text-slate-500 block mb-1.5">Material Grade</label>
                     <input 
                        className="erp-input"
                        placeholder="e.g. SS 304, CRCA"
                  ...
                  <div className="grid grid-cols-3 gap-2 col-span-2">
                     <div>
                        <label className="text-xs font-semibold text-slate-500 block mb-1.5">Thickness (mm)</label>
                        <input 
                     ...
                     <div>
                        <label className="text-xs font-semibold text-slate-500 block mb-1.5">Width (mm)</label>
                        <input 
                           type="number" step="1"
                           className="erp-input font-bold"
                     ...
                     <div>
                        <label className="text-xs font-semibold text-slate-500 block mb-1.5">Length (mm)</label>
                        <input 
                           type="number" step="1"
                           className="erp-input font-bold"
            ...
            <div className="p-4 space-y-4">
               <div className="p-4 bg-slate-50 rounded-md border border-slate-200 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        <p className="text-xs font-semibold text-slate-500">Balance Left</p>
                     </div>
                  </div>
               ...
               <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                     <label className="text-xs font-semibold text-slate-500 block mb-1.5">Payment Note</label>
                     <input className="erp-input" placeholder="UTR / Bill Number" value={paymentNote} onChange={(e) => setPaymentNote(e.target.value)} />
                  </div>
               </div>
               ...
               <div className="space-y-4 relative before:absolute before:left-5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                  {paymentHistory.map((event, idx) => (
                     <div key={idx} className="relative pl-12">
                        ...
                        <div>
                           <p className="text-xs font-semibold text-slate-500 mb-0.5">{formatDate(event.date)} — {new Date(event.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
