...
const CompanyStatement = () => {
   ...
   const handleExport = () => {
      ...
      link.href = url;
      link.setAttribute("download", `Master_Statement_${formatDate(new Date())}.csv`);
   ...
   if (loading) return (
      <AppLayout>
         <div className="flex flex-col items-center justify-center h-[70vh]">
            <HammerLoader />
            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] mt-4 animate-pulse">Synchronizing Fiscal Ledger...</p>
         </div>
      </AppLayout>
   );
   ...
   const filteredTimeline = (statement?.timeline || []).filter(item => {
      ...
      const matchesSearch = item.description?.toLowerCase().includes(searchLow) ||
         String(item.ref || "").toLowerCase().includes(searchLow);

      const matchesType = filterType === "all" ||
         (filterType === "credit" && item.credit > 0) ||
         (filterType === "debit" && item.debit > 0);

      const itemDate = new Date(item.date).getTime();
      const matchesStart = !dateRange.start || itemDate >= new Date(dateRange.start).getTime();
      const matchesEnd = !dateRange.end || itemDate <= new Date(dateRange.end).getTime() + (86400000 - 1);
      ...
      `}</style>
         ...
         <div className="main-app-content space-y-3 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 pt-4">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h2 className="text-4xl font-black text-foreground tracking-tightest leading-none mb-2 ">Total <span className="text-primary">Balance Sheet</span></h2>
                     <div className="flex items-center gap-3">
                        <ShieldCheck className="w-4 h-4 text-indigo-500" />
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">Complete record of all transactions</span>
                     </div>
                  </div>
               </div>

               <div className="flex items-center gap-4">
                  <button onClick={() => window.print()} className="erp-button-secondary !py-3 !px-4 border-border rounded-md">
                     <Printer className="w-5 h-5" />
                     Print Report
                  </button>
                  <button onClick={handleExport} className="erp-button-primary !py-3 !bg-indigo-600 !text-white dark:!text-white !rounded-md hover:!bg-indigo-700 group shadow-lg shadow-indigo-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               <div className="p-4 bg-card rounded-md border border-border shadow-sm group hover:border-emerald-200 transition-all duration-500 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-5"><TrendingUp className="w-16 h-16 text-emerald-600" /></div>
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-4">Total Received</p>
               ...
               <div className="p-4 bg-card rounded-md border border-border shadow-sm group hover:border-rose-200 transition-all duration-500 relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-5"><TrendingDown className="w-16 h-16 text-rose-600" /></div>
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-4">Total Paid</p>
               ...
               <div className={`p-4 rounded-md shadow-2xl relative overflow-hidden group transition-all ${statement?.totalBalance >= 0 ? "bg-slate-900 shadow-slate-900/20" : "bg-rose-900 shadow-rose-900/20"}`}>
                  ...
                  <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-4">Current Balance</p>
            ...
            <div className="bg-card rounded-md border border-border shadow-sm overflow-hidden mb-20">
               <div className="p-4 border-b border-border flex flex-col md:flex-row justify-between items-center gap-4 bg-muted/20">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Report Period</h4>
                        <p className="text-sm font-black text-foreground uppercase  tracking-tighter">All-time history</p>
                     </div>
                  </div>
                  <div className="flex items-center gap-4">
                     <div className="relative group">
                        <Search className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors ${searchTerm ? 'text-foreground' : 'text-muted-foreground'}`} />
                        <input
                           placeholder="Find transaction..."
                           className="pl-12 pr-4 py-4 bg-card border border-border rounded-md text-[10px] font-black uppercase tracking-widest outline-none focus:ring-4 focus:ring-slate-900/5 focus:border-slate-300 transition-all w-72"
                           value={searchTerm}
                     ...
                     <button
                        ...
                        className={`p-4 rounded-md transition-all ${showFilters ? 'bg-slate-900 text-white shadow-xl shadow-slate-900/40' : 'bg-card text-muted-foreground border border-border hover:text-foreground'}`}
                     >
               ...
               <AnimatePresence>
                  {showFilters && (
                     <motion.div
                        ...
                        className="overflow-hidden bg-muted/50 border-b border-border"
                     >
                        <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                           <div className="space-y-2">
                              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Transaction Flow</p>
                              <div className="flex gap-2">
                                 {['all', 'credit', 'debit'].map(t => (
                                    <button
                                       key={t}
                                       onClick={() => setFilterType(t)}
                                       className={`px-4 py-2 rounded-md text-[10px] font-black uppercase tracking-widest border transition-all ${filterType === t ? 'bg-slate-900 text-white border-slate-900 shadow-md' : 'bg-card text-muted-foreground border-border hover:border-slate-300'}`}
                                    >
                                       {t === 'all' ? 'Unified' : t === 'credit' ? 'Income' : 'Expense'}
                                    </button>
                                 ))}
                              </div>
                           </div>
                           ...
                           <div className="space-y-2">
                              ...
                              <input
                                 ...
                                 className="w-full px-4 py-2 bg-card border border-border rounded-md text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-slate-900/5"
                              />
                           </div>
                           ...
                           <div className="flex items-end">
                              <button
                                 onClick={() => { setFilterType("all"); setDateRange({ start: "", end: "" }); setSearchTerm(""); }}
                                 className="w-full py-2 bg-card border border-border rounded-md text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-rose-500 hover:border-rose-200 transition-all"
                              >
                                 Reset Engine
                              </button>
                           </div>
               ...
               <div className="overflow-x-auto">
                  <table className="erp-table">
                     <thead>
                        <tr className="bg-muted/50/50 text-[10px] font-black uppercase text-muted-foreground tracking-[0.2em] border-b border-slate-50">
                           <th className="px-10 py-3">Date & Time</th>
                           <th className="px-10 py-3">Description</th>
                           <th className="px-10 py-3 text-right">Paid</th>
                           <th className="px-10 py-3 text-right">Received</th>
                              ...
                              <tr key={idx} className="group erp-row-hover transition-all duration-500">
                                 <td className="px-10 py-3">
                                    <div className="flex flex-col">
                                       ...
                                       <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest mt-1 opacity-40">{new Date(item.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                 </td>
                                 <td className="px-10 py-3 max-w-sm">
                                    <div className="flex flex-col">
                                       <span className="text-sm font-black text-foreground tracking-tightest group-hover:text-slate-600 transition-colors uppercase ">{item.description}</span>
                                 ...
                                 <td className="px-10 py-3 text-right pr-20">
                                    <div className="flex flex-col items-end">
                                       <span className={`text-base font-black tabular-nums  ${item.balance >= 0 ? "text-foreground text-glow" : "text-rose-900 underline decoration-rose-200 decoration-4"}`}>
         ...
         <div className="printable-document font-sans">
            <div className="text-center border-b-4 border-black pb-4 mb-10">
               ...
               <div className="mt-4 py-2 bg-black text-white inline-block px-4 rounded-md text-[10px] font-black uppercase tracking-widest">Master Financial Audit Report</div>
            </div>
            ...
            <table className="erp-table">
               ...
               <tbody>
                  {statement?.timeline?.map((item, idx) => (
                     <tr key={idx}>
                        <td className="font-bold">{formatDate(item.date)}</td>
                        <td>
