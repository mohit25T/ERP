         ...
         <div className="space-y-3 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 pt-4">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h2 className="text-4xl font-black text-slate-900 tracking-tightest leading-none mb-2 ">Workforce <span className="text-primary not-">Intelligence</span></h2>
               ...
               <div className="flex items-center p-1.5 bg-slate-100 rounded-md border border-slate-200">
                  <button onClick={() => setActiveTab("staff")} className={`flex items-center gap-2 px-4 py-3 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === "staff" ? 'bg-white text-slate-900 shadow-lg scale-105' : 'text-slate-400 hover:text-slate-600'}`}>
                     <UserCheck className="w-4 h-4" /> Workforce Registry
                  </button>
                  <button onClick={() => setActiveTab("payroll")} className={`flex items-center gap-2 px-4 py-3 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === "payroll" ? 'bg-white text-slate-900 shadow-lg scale-105' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            {/* Dynamic Toolbar */}
            <div className="flex flex-wrap gap-4 items-center justify-between bg-white p-4 rounded-md border border-slate-100 shadow-sm">
               <div className="flex items-center gap-4">
                  {activeTab === "payroll" && (
                     <>
                        <select className="px-4 py-4 bg-white border border-slate-100 rounded-md text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-slate-900/10 cursor-pointer shadow-sm" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)}>
                           {months.map(m => <option key={m} value={m}>{m}</option>)}
                        </select>
                        <select className="px-4 py-4 bg-white border border-slate-100 rounded-md text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-slate-900/10 cursor-pointer shadow-sm" value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)}>
            ...
            {loading ? (
               <div className="flex flex-col items-center justify-center h-[40vh]">
                  <HammerLoader />
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-4 animate-pulse">Syncing Workforce Data...</p>
               </div>
            ) : activeTab === "staff" ? (
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {staff.length === 0 ? (
                     <div className="col-span-full py-40 text-center bg-slate-50/50 rounded-md border-2 border-dashed border-slate-200 flex flex-col items-center gap-4">
                     ...
                     <div key={s._id} className="group bg-white rounded-md p-4 border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-900/10 transition-all duration-700 hover:-translate-y-2 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 mb-10">
                           ...
                           <div>
                              <h4 className="text-2xl font-black text-slate-900 tracking-tightest uppercase  leading-none mb-2">{s.name}</h4>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t border-slate-50 pt-10">
                           <div className="space-y-1">
                              ...
                              <p className="text-2xl font-black text-slate-900 tracking-tightest ">₹{s.baseSalary.toLocaleString()}</p>
                              ...
                              {filteredPayrollEntries.map((p, index) => (
                                 ...
                                 >
                                    <td className="px-10 py-3">
                                       <div className="flex items-center gap-4">
                                          <div className="w-12 h-12 bg-white rounded-md flex items-center justify-center text-slate-900 font-black text-lg  shadow-lg border border-slate-100 group-hover:scale-110 transition-transform">
                                             {p.staff?.name.charAt(0)}
                                          </div>
                                          <div>
                                             <p className="text-base font-black text-slate-900 tracking-tightest uppercase  leading-none mb-1.5">{p.staff?.name}</p>
                                    ...
                                    <td className="px-10 py-3 text-center">
                                       <div className="inline-flex items-center gap-3 px-3 py-2.5 bg-slate-50 rounded-md border border-slate-100 shadow-inner">
                                    ...
                                    <td className="px-10 py-3 text-right">
                                       <p className="text-2xl font-black text-slate-900 tracking-tightest  tabular-nums leading-none">₹{p.netSalary.toLocaleString()}</p>
                                    ...
                                    <td className="px-10 py-3 text-right pr-20">
                                       {p.status === 'unpaid' ? (
                                          ...
                                          <div className="flex items-center justify-end gap-3 text-slate-400 font-black  uppercase tracking-widest text-[10px] bg-slate-50 px-4 py-3 rounded-md border border-slate-100 w-fit ml-auto">
            ...
            {/* MODAL CONFIG */}
            <Modal isOpen={isStaffModalOpen} onClose={() => setIsStaffModalOpen(false)} title={<div className="flex items-center gap-4"><UserPlus className="w-6 h-6 text-slate-900" /><span className="text-xl font-black  tracking-tightest uppercase">Personnel Registration</span></div>}>
               <form onSubmit={handleCreateStaff} className="space-y-3 p-4">
                  <div className="grid grid-cols-2 gap-4">
                     <div className="col-span-2 space-y-3">
                        ...
                        <input name="name" required defaultValue={editingStaff?.name} className="erp-input !py-3 !bg-slate-50 focus:!bg-white rounded-md" placeholder="Associate Name..." />
                     </div>
                     <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Operational Node (Mobile)</label>
                        <input name="mobile" required defaultValue={editingStaff?.mobile} className="erp-input !py-3 !bg-slate-50 font-mono tracking-widest rounded-md" placeholder="+91 XXXX XXX XXX" />
                     </div>
                     <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Designation / Role</label>
                     ...
                     <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Base Compensation (₹)</label>
                     ...
                     <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Yield Frequency</label>
                        <select name="salaryType" defaultValue={editingStaff?.salaryType || "monthly"} className="erp-input !py-3 uppercase !font-black cursor-pointer bg-slate-50 rounded-md">
                  ...
                  <div className="pt-4 border-t border-slate-100 flex gap-4">
                     <button type="button" onClick={() => setIsStaffModalOpen(false)} className="flex-1 py-3 text-[10px] font-black uppercase text-slate-400 hover:text-slate-900 tracking-widest ">Abort Protocol</button>
            ...
            <Modal isOpen={isPayrollModalOpen} onClose={() => setIsPayrollModalOpen(false)} title={<div className="flex items-center gap-4"><Banknote className="w-6 h-6 text-slate-900" /><span className="text-xl font-black  tracking-tightest uppercase">Initialize Dispersion</span></div>}>
               <form onSubmit={handleGeneratePayroll} className="space-y-3 p-4">
                  <div className="space-y-3">
                     <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Target Associate</label>
                     <select name="staffId" required className="erp-input !py-3 uppercase !font-black cursor-pointer bg-slate-50 rounded-md">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Operational Units</label>
                     ...
                     <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Growth Bonus (₹)</label>
                        <input name="bonus" type="number" defaultValue="0" className="erp-input !py-3 !text-emerald-600 !font-black !text-lg rounded-md" />
                     </div>
                     <div className="col-span-2 space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-2 ">Operational Deductions (₹)</label>
                        <input name="deductions" type="number" defaultValue="0" className="erp-input !py-3 !text-rose-600 !font-black !text-lg rounded-md" />
                     </div>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-md border border-slate-100 flex items-center gap-4 shadow-inner">
                     <Activity className="w-8 h-8 text-slate-300" />
                     <p className="text-[10px] font-black uppercase text-slate-400 leading-relaxed ">Calculations synchronized with daily/monthly performance protocols.</p>
                  </div>
