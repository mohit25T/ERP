...
const PublicLedger = () => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 animate-pulse">
       <div className="w-14 h-14 bg-slate-900 rounded-[2rem] flex items-center justify-center mb-4 shadow-2xl">
          <Globe className="w-7 h-7 text-white animate-spin-slow" />
       </div>
       <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] ">Accessing Transparency Hub...</p>
    </div>
  );

  if (error) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white p-6 text-center">
       <div className="w-24 h-24 bg-rose-50 text-rose-500 rounded-[2.5rem] flex items-center justify-center mb-4 border border-rose-100 shadow-xl shadow-rose-500/10">
          <Lock className="w-7 h-7" />
       </div>
       <h1 className="text-2xl font-black text-slate-900 tracking-tightest leading-none mb-4 ">Security <span className="text-rose-500 border-b-4 border-rose-500">Lock</span></h1>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 pb-24">
      ...
      <nav className="bg-white/80 backdrop-blur-3xl border-b border-slate-100 px-10 py-4 flex justify-between items-center sticky top-0 z-[100] shadow-sm">
         <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
               ...
               <span className="text-sm font-black text-slate-900 tracking-tightest uppercase ">Secure Guest Reconciliation</span>
            </div>
         </div>
         <div className="flex items-center gap-4">
      ...
      <div className="max-w-2xl mx-auto px-10 py-16 space-y-4 animate-in fade-in slide-in-from-bottom-5 duration-1000">
         ...
         <div className="text-center space-y-4">
            ...
            <div>
               <h1 className="text-2xl font-black text-slate-900 tracking-tightest uppercase  mb-2 leading-none">
               ...
               <div className="flex flex-col items-center justify-center text-center">
                  ...
                  <div className="mt-4 px-4 py-3 bg-white border border-slate-100 rounded-[2rem] shadow-sm transform hover:scale-105 transition-transform">
                     <span className="text-xl font-black text-slate-900 tracking-tightest uppercase  border-b-2 border-indigo-500">{party.company || party.name}</span>
                  </div>
               </div>
            </div>
         </div>
         ...
         <div className="bg-slate-900 p-6 rounded-[2rem] shadow-2xl shadow-slate-900/20 relative overflow-hidden group hover:scale-[1.02] transition-all duration-700">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-4 pl-1 border-l-4 border-indigo-500">Current Net Obligation</p>
            <div className="flex flex-col md:flex-row md:items-end gap-4 mb-2">
               ...
               <div className={`px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest  border mb-1 w-fit ${totalOutstanding > 0 ? "bg-rose-500/10 text-rose-400 border-rose-500/20" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"}`}>
            ...
            <div className="mt-12 pt-10 border-t border-slate-800 flex flex-col md:flex-row gap-4 print:hidden">
               {type === 'customer' && (
                  <a href={upiLink} className="flex-1 bg-white text-slate-900 py-4 px-10 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-xl hover:scale-105 transition-all group active:scale-95 leading-none">
                     <CreditCard className="w-5 h-5 group-hover:rotate-12 transition-transform" /> 
                     Synchronized UPI Payment
                  </a>
               )}
               <button onClick={() => window.print()} className="flex-1 bg-slate-800 text-slate-400 py-4 px-10 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-slate-700 hover:text-white transition-all active:scale-95 leading-none">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-500">
               ...
               <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                     <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 ">Total Billed Yield</p>
                     <p className="text-2xl font-black text-slate-900 tracking-tightest  tabular-nums leading-none">₹{Number(totalDebit || 0).toLocaleString()}</p>
                  </div>
               </div>
            ...
            <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-500">
               ...
               <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                     <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 ">Settled Liquidity</p>
                     <p className="text-2xl font-black text-slate-900 tracking-tightest  tabular-nums leading-none">₹{Number(totalCredit || 0).toLocaleString()}</p>
                  </div>
               </div>
         ...
         <div className="space-y-4">
            <div className="flex items-center gap-4">
                ...
                <h3 className="text-[11px] font-black uppercase text-slate-900 tracking-[0.3em] ">Historical Transaction Log</h3>
            </div>
            
            <div className="space-y-3">
               {timeline.slice().reverse().map((item, idx) => (
                    <div key={idx} className="bg-white p-4 rounded-[2.5rem] border border-slate-100 flex justify-between items-center group hover:border-indigo-200 hover:shadow-2xl hover:shadow-indigo-500/5 transition-all duration-500">
                       <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-2 ">Ref_Node: {new Date(item.date).toLocaleDateString()}</span>
                       ...
                       <div className="text-right">
                          ...
                          <div className={`inline-flex px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest  ${item.type === 'payment' ? 'bg-emerald-50 text-emerald-500 border border-emerald-100' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>
      ...
      <div className="printable-document hidden print:block bg-white p-6 min-h-screen">
         <div className="flex justify-between items-start border-b-2 border-slate-900 pb-4 mb-10">
            <div>
               <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase ">{companyInfo?.name || "Corporate Integrity Node"}</h1>
            ...
            <div className="text-right">
               <div className="inline-block bg-slate-900 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest  mb-4">Official Audit Statement</div>
         ...
         <div className="grid grid-cols-2 gap-3 mb-12">
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 ">Authorized Party</p>
               <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight ">{party.company || party.name}</h2>
               <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-2">{party.address}</p>
            </div>
            <div className="bg-slate-900 p-4 rounded-2xl text-white">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 ">Net Reconciliation Balance</p>
            ...
            <tbody>
               {timeline.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-50">
                     ...
                     <td className="py-4 text-[11px] font-black text-slate-900 uppercase ">{item.description}</td>
         ...
         <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 mt-20">
            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest  max-w-xs leading-loose">
            ...
            <div className="text-right">
               <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest  mb-2">Verified Reconciliation Node</p>
