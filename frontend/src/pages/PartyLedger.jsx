        ...
        <div className="space-y-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map(p => (
                 <a 
                   key={p._id}
                   href={`/reports/party/${p._id}?type=${p.type}`}
                       ...
                       statement?.timeline.map((item, idx) => (
                          <tr key={idx} className="hover:bg-gray-50/50 transition-colors group">
                             <td className="px-8 py-6">
                                <span className="text-sm font-black text-gray-900 italic tracking-tight">{new Date(item.date).toLocaleDateString()}</span>
                             </td>
                             <td className="px-8 py-6">
                             ...
                             <td className="px-8 py-6 text-right">
                                {item.debit > 0 && (
                                   <span className="text-sm font-black text-red-600 tabular-nums">₹{item.debit.toLocaleString()}</span>
                                )}
                             </td>
                             <td className="px-8 py-6 text-right">
                                {item.credit > 0 && (
                                   <span className="text-sm font-black text-green-600 tabular-nums">₹{item.credit.toLocaleString()}</span>
                                )}
                             </td>
                             <td className="px-8 py-6 text-right pr-12">
                                <span className="text-md font-black text-gray-900 tabular-nums italic">
