...
import {
   FileText, ArrowLeft, Download, Calendar, User, Building2,
   ArrowUpRight, ArrowDownRight, Printer, Wallet, Share2, Zap,
   Search, CheckCircle2, AlertCircle, Loader2, Users, Activity,
   ShieldCheck, Database, ChevronRight, TrendingUp, History
} from "lucide-react";
            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
               <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                  <div className="flex items-center gap-6">
                     <div className="w-20 h-20 bg-slate-100 rounded-[2.5rem] flex items-center justify-center group hover:scale-110 transition-transform duration-500 shadow-sm border border-slate-200">
                        <Users className="w-10 h-10 text-slate-900" />
                     </div>
                     <div>
                        <h2 className="text-5xl font-black text-slate-900 tracking-tightest leading-none mb-2 italic">Party <span className="text-slate-400 not-italic">Ledger Hub</span></h2>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  {filteredParties.map(p => (
                     <Link key={p._id} to={`/statements/${p._id}?type=${p.type}`} className="group bg-white rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 hover:border-slate-900/10 transition-all duration-500 overflow-hidden">
   ...
   return (
      <AppLayout>
         ...
         <div className="main-app-content space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                     <h2 className="text-5xl font-black text-slate-900 tracking-tightest leading-none mb-2 italic">Party <span className="text-slate-400 not-italic">Ledger</span></h2>
                     <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
               <div className="lg:col-span-2 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div className="p-10 bg-white rounded-[3rem] border border-slate-100 shadow-sm group relative overflow-hidden">
                        ...
                        <h3 className="text-4xl font-black text-indigo-600 tracking-tightest tabular-nums italic">₹{Number(statement?.totalDebit || 0).toLocaleString()}</h3>
                     </div>
                     <div className="p-10 bg-white rounded-[3rem] border border-slate-100 shadow-sm group relative overflow-hidden">
                        ...
                        <h3 className="text-4xl font-black text-emerald-600 tracking-tightest tabular-nums italic">₹{Number(statement?.totalCredit || 0).toLocaleString()}</h3>
                     </div>
                  </div>

                  <div className="bg-white rounded-[4rem] border border-slate-100 shadow-sm overflow-hidden min-h-[600px]">
                     <div className="p-10 border-b border-slate-50 flex items-center justify-between">
                        ...
                        <div className="flex -space-x-3">
                           {[1,2,3].map(i => <div key={i} className="w-10 h-10 rounded-full border-4 border-white bg-slate-100 text-[10px] font-black flex items-center justify-center text-slate-400 italic">TX</div>)}
                        </div>
                     </div>
                        ...
                        <table className="w-full text-left">
                           <thead>
                              <tr className="bg-slate-50/50 text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] border-b border-slate-50">
                                 ...
                                 <th className="px-12 py-8">Account Narrative</th>
                                 <th className="px-12 py-8 text-right">MAGNITUDE (₹)</th>
                                 <th className="px-12 py-8 text-right pr-20">FISCAL CLOSURE</th>
                              </tr>
                           </thead>
                                 ...
                                 <tr key={idx} className="group hover:bg-slate-50/80 transition-all duration-500">
                                    ...
                                    <td className="px-12 py-8">
                                       <div className="flex flex-col">
                                          ...
                                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1.5 opacity-40">REF: {item.ref.slice(-8).toUpperCase()}</span>
                                       </div>
                                    </td>
               ...
               <div className="space-y-8">
                  <div className={`p-10 rounded-[3.5rem] shadow-2xl relative overflow-hidden transition-all duration-700 ${statement?.totalOutstanding > 0 ? "bg-rose-900 shadow-rose-900/20 text-white" : "bg-emerald-900 shadow-emerald-900/20 text-white"}`}>
                     ...
                     <h2 className="text-5xl font-black italic tracking-tightest leading-none">₹{Math.abs(statement?.totalOutstanding || 0).toLocaleString()}</h2>
                  ...
                  <div className="p-10 bg-white rounded-[3.5rem] border border-slate-100 shadow-sm space-y-6">
         ...
         <Modal isOpen={isReconcileOpen} onClose={() => setIsReconcileOpen(false)} title="Treasury: Account Reconciliation">
            <form onSubmit={handleReconcileSubmit} className="p-10 space-y-8">
               <div className="p-10 bg-emerald-50 rounded-[3rem] border border-emerald-100 flex gap-6 items-center">
               ...
               <div>
                  ...
                  <input type="number" required value={reconcileAmount} onChange={e => setReconcileAmount(e.target.value)} className="erp-input !py-8 !text-5xl font-black !text-slate-900 tabular-nums" />
               </div>
               <div>
